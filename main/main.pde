...
void draw() {
  ...
  if (bossScreen) {
    if (cannons.size() < 6) {
      bgMusic.stop();
          bgMusic = new SoundFile(this, "../AudioFiles/boss.mp3");
          bgMusic.play();
          bgMusic.loop();
      for (int i = 0; i < 6; i++) {
    ...
    fill(color(255, 0, 0));
    PImage bossSprite = loadImage("../Images/oscar.jpg");
    image(bossSprite,width / 2.0f - 50.0f, height / 2.0f - 50.0f, 100.0f, 100.0f);
    PC.Movement(mode);
    PC.posCheck();
    PC.Draw();
